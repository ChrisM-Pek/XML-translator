...
LMSTUDIO_API_KEY  = os.getenv("LMSTUDIO_API_KEY", "lm-studio")  # optionnel
...
# ===== XML helpers =====
def make_xml_parser():
    # Tolérant : entités incomplètes (&), multiples racines/artefacts, gros fichiers
    return etree.XMLParser(remove_blank_text=False, recover=True, huge_tree=True, resolve_entities=False)

# ===== Post-traitement langue =====
def strip_accents(s: str) -> str:
    # Supprime les marques diacritiques (accents) Unicode
    return ''.join(c for c in unicodedata.normalize('NFD', s) if unicodedata.category(c) != 'Mn')

def postprocess_output_for_lang(text: str, target_lang: str) -> str:
    # Si langue cible est le français, enlever tous les accents
...
def choose_engine() -> str:
    ...
    try:
        ...
        if LMSTUDIO_API_KEY: headers["Authorization"] = f"Bearer {LMSTUDIO_API_KEY}"
...
def translate_lmstudio(text: str, target_lang: str="FR", source_lang: Optional[str]=None) -> str:
    ...
    out = postprocess_output_for_lang(out, target_lang)  # <<< suppression accents FR
    return out

def translate_deepl(text: str, target_lang: str="FR", source_lang: Optional[str]=None) -> str:
    ...
    if source_lang: payload["source_lang"] = source_lang.upper()
...
def translate_text(text: str, target_lang: str="FR", source_lang: Optional[str]=None) -> str:
    if not text or text.strip() == "":
        return text
    eng = choose_engine()
    if eng == "lmstudio":
        try:
            out = translate_lmstudio(text, target_lang, source_lang)
        except Exception as e:
            if os.getenv("DEEPL_API_KEY"):
                log(f"    [WARN] LM Studio rejeté ({e}); fallback DeepL.")
                return translate_deepl(text, target_lang, source_lang)
            raise
        return out
    else:
        return translate_deepl(text, target_lang, source_lang)
...
def count_text_chars_in_file(path: Path) -> Tuple[int, int]:
    ...
    total = 0
    count = 0
    for el in tree.getroot().iter("text"):
        s = (el.text or "").strip()
        if s:
            count += 1
            total += len(s)
    if count == 0:
        log(f"[PRESCAN:ZERO_TEXT] {path} : aucun <text> non vide trouvé")
    return count, total
...
def prescan_dir(in_dir: Path, out_dir: Optional[Path] = None) -> Tuple[int, int, Dict[Path, Tuple[int,int]]]:
    ...
    for f in files:
        ...
        file_stats[f] = (c, ch)
        total_chars += ch
    return len(files), total_chars, file_stats
...
def translate_xml_tree(root: etree._Element, file: Path, prog: Progress):
    for i, text_el in enumerate(root.iter("text"), start=1):
        ...
        t_start = time.time()
        try:
            translated = translate_text(original, target_lang=TARGET_LANG, source_lang=None)
            dt = time.time() - t_start
            text_el.text = translated
            prog.update(len(s), dt)
            log(f"    [OK] {file.name} <text #{i}>  "
...
def main():
    ...
    # --- Purge input: retirer tout fichier déjà présent dans output ---
    ...
    if nb_files == 0 or total_chars == 0:
        log("--- Rien à traduire (tout est déjà présent dans output) ---")
        sys.exit(0)
    ...
    for src in iter_xml_files(in_dir):
        ...
        _, file_chars = file_stats.get(src, (0, 0))
        try:
            translate_xml_file(src, dst, file_chars, prog)
            processed += 1

            # --- Suppression du fichier source si la traduction a réussi ---
            try:
                if dst.exists():
                    src.unlink()
